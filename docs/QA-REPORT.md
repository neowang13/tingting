...
- Vitest: 63 passed; 16 credential-dependent live-Supabase RLS checks skipped.
...
- PostgreSQL 17: migrations `202607240001` through `202607260013` applied from
...
- PostgreSQL 17 migrations through `202607260013` applied from an empty database
...
**Local build: SHIP. OpenClaw production enablement: gated by external
provisioning and launch checks.**
...
audit is clean. Pixel visual regression, live Supabase RLS HTTP tests,
production Core Web Vitals, provider dry runs, and verified real callbacks
remain external-environment gates.
