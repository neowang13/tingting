...
const baseEnvSchema = z.object({
  ...
  NEXT_PUBLIC_GOOGLE_REVIEW_URL: z.string().url().optional(),
  ...
  LOCAL_CLIENT_EMAIL: z.string().email().optional(),
  LOCAL_CLIENT_PASSWORD_HASH: z.string().min(1).optional(),
  LOCAL_CLIENT_SESSION_SECRET: z.string().min(32).optional(),
  LOCAL_CLIENT_DISPLAY_NAME: z.string().min(1).optional(),
  ...
  OWNER_WEEKLY_SUMMARY_DAY: z.enum(["1", "2", "3", "4", "5", "6", "7"]).default("1"),
  OWNER_WEEKLY_SUMMARY_TIME: z.string().regex(/^(?:[01]\d|2[0-3]):[0-5]\d$/).default("09:00"),
  OWNER_WEEKLY_SUMMARY_ENABLED: z.enum(["true", "false"]).default("false"),
...
export function isOwnerDailyOverdueEnabled(source: NodeJS.ProcessEnv = process.env) {
  return source.OWNER_DAILY_OVERDUE_ENABLED === "true";
}

export function isOwnerWeeklySummaryEnabled(source: NodeJS.ProcessEnv = process.env) {
  return source.OWNER_WEEKLY_SUMMARY_ENABLED === "true";
}
