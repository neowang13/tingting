...
export interface ApplicationNotificationApplicant extends ApplicationApplicantRecord {
...
function applicantCards(applicant: ApplicationNotificationApplicant, index: number) {
  ...
  const explanations = APPLICATION_REQUIRED_DOCUMENT_TYPES
    .filter((type) => draft.documentExplanations[type])
    .map((type) => `${APPLICATION_DOCUMENT_LABELS[type]}: ${draft.documentExplanations[type]}`)
    .join("\n") || "None";
  const role = applicant.role === "primary" ? "Primary applicant" : `Co-applicant ${index}`;
  ...
  return `<div style="margin-top:22px">
    ...
    ${card("Requested tenancy", [
      ...
      ["Adults", draft.tenancy.adultCount],
      ["Children", draft.tenancy.childCount],
...
function textApplicant(applicant: ApplicationNotificationApplicant, index: number) {
  ...
  const rows: Array<[string, string | number | null | undefined]> = [
    ...
    ["Lease term", humanize(draft.tenancy.leaseTerm)], ["Adults", draft.tenancy.adultCount],
    ["Children", draft.tenancy.childCount], ["Total occupants", totalOccupants],
    ["Needs showing", humanize(draft.tenancy.needsShowing)],
    ["Pets", draft.tenancy.hasPets ? draft.tenancy.petDetails || "Yes" : "No"],
    ["Parking", draft.tenancy.needsParking ? "Required" : "Not required"],
  ...
  const explanations = APPLICATION_REQUIRED_DOCUMENT_TYPES
    .filter((type) => draft.documentExplanations[type])
    .map((type) => `${APPLICATION_DOCUMENT_LABELS[type]}: ${draft.documentExplanations[type]}`);
  return [role, ...rows.map(([label, value]) => `${label}: ${display(value)}`), "Document explanations:", ...(explanations.length ? explanations.map((value) => `- ${value}`) : ["- None"])].join("\n");
}
...
export function applicationEmailAttachmentFilename(
  ...
  const role = applicant?.role === "primary" ? "primary" : "co-applicant";
...
export function renderApplicationSubmittedNotification(input: {
  ...
  const attachedRows = input.attachedFiles.map((file) => {
    const applicant = applicants.find((candidate) => candidate.id === file.applicantId);
    return `<tr><td style="border-bottom:1px solid #ECE9E3;padding:10px 8px 10px 0"><strong>${escapeHtml(file.originalFilename)}</strong><br><span style="color:#6B6F6D;font-size:12px">${escapeHtml(APPLICATION_DOCUMENT_LABELS[file.documentType])}</span></td><td style="border-bottom:1px solid #ECE9E3;padding:10px 0;text-align:right;vertical-align:top">${escapeHtml(applicant ? applicantDisplayName(applicant) : "Applicant")}<br><span style="color:#6B6F6D;font-size:12px">${Math.ceil(file.byteSize / 1024)} KB</span></td></tr>`;
  }).join("");
  ...
  const textFiles = input.attachedFiles.length
    ...
    : ["- No uploaded files; required-document explanations are included below."];
  const text = [
