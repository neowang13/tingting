-- Repair environments where application-to-tenant conversion was deployed
-- without the client/tenant link objects it depends on. Keep this migration
-- idempotent so it is safe for environments that already applied 043.

create table if not exists public.client_tenant_links (
...
create unique index if not exists client_tenant_links_one_current_idx
  on public.client_tenant_links(client_user_id)
  where archived_at is null;

create index if not exists client_tenant_links_history_idx
  on public.client_tenant_links(client_user_id, linked_at desc);

create index if not exists client_tenant_links_current_tenant_idx
...
revoke all on function public.admin_link_client_tenant(uuid, uuid, uuid)
  from public, anon, authenticated;
revoke all on function public.admin_unlink_client_tenant(uuid, uuid)
  from public, anon, authenticated;
grant execute on function public.admin_link_client_tenant(uuid, uuid, uuid)
  to service_role;
grant execute on function public.admin_unlink_client_tenant(uuid, uuid)
  to service_role;
...
revoke all on function public.archive_client_links_for_tenant()
  from public, anon, authenticated;
